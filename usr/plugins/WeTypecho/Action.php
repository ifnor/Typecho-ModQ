...
class WeTypecho_Action extends Typecho_Widget implements Widget_Interface_Do {
    ...
    private function posts() {
        ...
        foreach ($posts as $post) {
            ...
            $post['thumb'] = $this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid']))?$this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid'])):array(array("str_value"=>"https://nk.uwp.ac.cn/bg.png"));
    ...
    private function getpersonalcid()
    {
        $cid = 'none';
        $cid = Typecho_Widget::widget('Widget_Options')->plugin('WeTypecho')->personalCid;
  ...
  //   }
    ...
    private function addcomment()
    {
        ...
        $coid =$this->db->query($this->db->insert('table.comments')->rows(array('cid' => $cid, 'created' => time(), 'author' => $author, 'authorId' => '0',
                'ownerId' => '1', 'mail' => 'wx@wx.com', 'url' => $openid, 'ip' => $ip, 'agent' => 'wx-miniprogram', 'text' => $text/*"写的真好，赞一个！"*/ , 'type' => 'comment',
            ...
            if(sizeof($cids)>0){
                foreach($cids as $cid) {
                    ...
                    if(sizeof($post)>0 && $post[0]!=null) {
                        ...
                        $post[0]['thumb'] = $this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $cid))?$this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $cid)):array(array("str_value"=>"https://nk.uwp.ac.cn/bg.png"));
