...
class WechatFans_Plugin implements Typecho_Plugin_Interface{
    ...
    public static function config(Typecho_Widget_Helper_Form $form){
        ...
        $wechat_name = new Typecho_Widget_Helper_Form_Element_Text('wechat_name', array("value"), '短信息', _t('微信公众号名称'), _t('微信公众号平台→公众号设置→名称，例如：短信息'));
        $form->addInput($wechat_name);
		$wechat_account = new Typecho_Widget_Helper_Form_Element_Text('wechat_account', array("value"), 'ceshon', _t('微信公众号名称'), _t(' 微信公众号平台→公众号设置→微信号，例如：ceshon'));
		...
		$wechat_qrimg = new Typecho_Widget_Helper_Form_Element_Text('wechat_qrimg', array("value"), 'https://ifdo.ml/pub.png', _t('微信公众号二维码地址'), _t('填写您的微信公众号的二维码图片地址，建议150X150像素'));
    ...
    public static function parseContent($obj){
		...
		if (preg_match_all($wechatfansRule, $content, $hide_words)){
			...
			if($cookievalue==$cv){
				$content = str_replace($hide_words[0], '<div>'.$hide_words[0][0].'</div>', $content);	
				//style="border:1px dashed #F60; padding:0px; margin:0px 0px; line-height:200%;  background-color:#FFF4FF; overflow:hidden; clear:both; width:640px; height:360px;"
			}else{
